...
* The idea is to create a pool market which encourages generators to bid based on realistic marginal cost, used to create a supply curve, while demand curve is often generated by load forecasting. The two curves are overlaid to find the intersection, which is used as a "system marginal price" (SMP), which is the price at which all generators who bid at or below SMP will be paid. [Kirschen, ch 3][Kirschen]
